            ...
            try:
                ...
                for chunk in reader:
                    ...
                    if operation == "Upload Master Inventory":
                        # Ensure columns match: part_number, quantity_on_hand, location_bin
                        ...
                        # We force the columns to match the DB schema exactly
                        # DB expects: part_number, description, quantity, employee_id, movement_type, location_bin, variance_amount, severity_level, timestamp
                        data = chunk.values.tolist()
                        execute_values(cur, """
                ...
                st.success("✅ Import Complete! Data is now live.")
                conn.close()
            except Exception as e:
                st.error(f"Upload Error: {e}")
                st.info("Check: Does your CSV have exactly 9 columns for Activity Log or 3 for Inventory?")
